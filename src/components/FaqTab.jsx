        ...
        <section className='qqq:py-[128px] eee:py-[50px] py-8 bg-white z-0'>
            <div className='containerb'>
                <div className='flex justify-center'>
                    <ul className='aaa:flex flex-wrap items-center   aaa:space-x-8 justify-center mb-12'>
                        <li>
                ...
                <ul className='grid md:grid-cols-2 grid-cols-1 gap-x-[34px] gap-y-6'>
                    {faqdata.map((faq) => (
                        <li className={`${activeTab === '' || activeTab === faq.theme ? "" : "hidden"} w-full max-w-[623px] mx-auto`} key={faq.id}>
                           <div>
                          <div className='flex md:block justify-center'>
                          <img className='border-[33px] border-[#98B1C6] mb-6 w-full max-w-[545px]' src={faq.img} alt={faq.title} />
                          </div>
                            <h3 className='text-#282938 text-xl text-center md:text-left  270px:text-[38px] font-semibold leading-[56px] mb-3'>{faq.title}</h3>
                            <p className='text-#282938 text-xs 270px:text-base leading-5 text-center md:text-left 270px:leading-7 mb-6'>{faq.text}</p>
                            <Link onClick={handleButtonClick} to={`/users/${faq.id}`} className='flex items-center text-#282938 text-base font-medium leading-6 md:justify-normal justify-center'>
                                Read case study
