        ...
        <ul className='grid md:grid-cols-2 grid-cols-1 tabletmax:grid-cols-3  gap-x-8 gap-y-16 '>
          {
            blogData.map((blog) => {
              return (
                <li key={blog.id} className='w-full max-w-[405px] mx-auto md:mx-0'>
                  <img className='w-full max-w-[405px] mb-8' src={blog.img} alt="blog-img" />
                  <span className='text-#282938 text-sm  ddd:text-base font-medium leading-3 ddd:leading-7 mb-4 opacity-[0.7]'>{blog.numberdata}</span>
                  <h3 className='text-#282938 text-lg  ddd:text-2xl font-medium leading-6 ddd:leading-9 mb-4'>{blog.title}</h3>
                  <p className='text-#282938 text-sm  ddd:text-base leading-5 ddd:leading-7 opacity-[0.7] mb-6'>{blog.text}</p>
